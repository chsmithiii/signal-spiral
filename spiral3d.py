...
def line3d_with_colormap(ax, x, y, z, color_values):
    ...
    # color by the mid-point's z (time) to emulate NASA color progression
    mids = (z[:-1] + z[1:]) / 2.0
    lc.set_array(mids)
...
def main():
    ...
    df = df[[args.date_col, args.value_col]].dropna()
    ...
    lc = line3d_with_colormap(ax, x, y, z, v)
    ...
    ax.grid(False)
    # subtle reference cylinder: draw faint circle at median radius on z=0
